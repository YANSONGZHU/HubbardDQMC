...
Suppose we have 4 B matrices corresponding time slices, and Numerical
Stability is performed every time slices. Then, we'll have following scheme.

B_up_β_τ[5] = I							B_up_τ_0[5] = B_4 * B_3 * B2 * B_1									...
B_up_β_τ[4] = B_4						B_up_τ_0[4] = B_3 * B2 * B_1
B_up_β_τ[3] = B_4 * B_3					B_up_τ_0[3] = B_2 * B_1
B_up_β_τ[2] = B_4 * B_3 * B_2			B_up_τ_0[2] = B_1
B_up_β_τ[1] = B_4 * B_3 * B_2 * B_1		B_up_τ_0[1] = I

Usually, there should be exactly Nt elements to store. However, we here
...
struct persistent{T <: Number}
	...
	function persistent(T::Type, Nt::Int, MatDim::Int)
		B_τ_0_up_udv = Vector{SVD_Store{T}}(undef, Nt + 1)
		B_τ_0_dn_udv = Vector{SVD_Store{T}}(undef, Nt + 1)
		B_β_τ_up_udv = Vector{SVD_Store{T}}(undef, Nt + 1)
		B_β_τ_dn_udv = Vector{SVD_Store{T}}(undef, Nt + 1)
